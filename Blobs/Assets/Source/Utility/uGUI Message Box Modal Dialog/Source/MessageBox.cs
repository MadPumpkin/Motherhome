...
public class MessageBox : ModalBox
{
    ...
    void SetUpButtons(MessageBoxButtons buttons)
    {
        ...
        switch (buttons)
        {
            case MessageBoxButtons.OK:
                button.GetComponentInChildren<Text>().text = Localize("OK"._());
            ...
            case MessageBoxButtons.OKCancel:
                ...
                CreateButton(button, Localize("Cancel"._()), () => { result = DialogResult.Cancel; Close(); });
                break;
            case MessageBoxButtons.YesNo:
                button.GetComponentInChildren<Text>().text = Localize("Yes"._());
                button.GetComponent<Button>().onClick.AddListener(() => { result = DialogResult.Yes; Close(); });

                CreateButton(button, Localize("No"._()), () => { result = DialogResult.No; Close(); });
                break;
            case MessageBoxButtons.RetryCancel:
                button.GetComponentInChildren<Text>().text = Localize("Retry"._());
            ...
            case MessageBoxButtons.AbortRetryIgnore:
                button.GetComponentInChildren<Text>().text = Localize("Abort"._());
                button.GetComponent<Button>().onClick.AddListener(() => { result = DialogResult.Abort; Close(); });

                CreateButton(button, Localize("Retry"._()), () => { result = DialogResult.Retry; Close(); });
                CreateButton(button, Localize("Ignore"._()), () => { result = DialogResult.Ignore; Close(); });
                break;
        }
    }
    ...
    {
        var result = Instantiate(buttonToClone) as GameObject;

        result.transform.SetParent(buttonToClone.transform.parent, false);

        result.GetComponentInChildren<Text>().text = label;

        var button = result.GetComponent<Button>();
        button.onClick.AddListener(target);
        button.Select();
